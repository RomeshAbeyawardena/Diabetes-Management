...
public class Sign : IRequestHandler<SignRequest, string>
{
    ...
    public Sign(IMediator mediator, IClockProvider clockProvider, ApplicationSettings applicationSettings)
    {
    ...
    public async Task<string> Handle(SignRequest request, CancellationToken cancellationToken)
    {
        ...
        if (request.Validate && Guid.TryParse(request.ApiKey, out var key))
        {
            ...
            if (accessToken != null)
            {
                ...
                else if (!string.IsNullOrWhiteSpace(applicationInstance.AccessToken))
                {
                    return applicationInstance.AccessToken;
                }
        ...
        var token = await mediator.Send(new JwtFeature.SignRequest { Dictionary = jwtdict }, cancellationToken);
